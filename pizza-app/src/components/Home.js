...
class Home extends Component {
  ...
  componentDidMount(){
      AxiosAPI.zipcode().then((res)=>{
       this.setState({postalcode : res});
      });
  }
  ...
  handlePostcodeSubmit = () => {
   // const formattedPostcode = this.state.postcode.toUpperCase().replace(/\s/g, "");
    let postal = false;
    const formattedPostcode = this.state.postcode;
    this.state.postalcode.forEach(element =>{
        if(element.value === formattedPostcode){
            postal = true;
        }
    });
    if(postal) {
  ...
  render(){
    ...
    return(
      <div id='home-page'>
        <Container >
          <Header as='h1' id="home-logo">Pizzeria</Header>
          <Container id="home-content">
            ...
            <Form size='large' onSubmit={this.handlePostcodeSubmit} fluid='true'>
              ...
              <Label color='orange' size='medium' id='home-label'><span role='img' aria-label='point-right'>👉</span> Hint: We only deliver to postcode '56001' right now</Label>
            </Form>
          </Container>
        </Container>
